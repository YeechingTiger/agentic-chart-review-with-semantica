...
from .corpus import Corpus, DocMeta
...
class LabelStore:
    ...
    def spend(self) -> float:
        ...
        (`acr.state` documents making exactly that mistake), so there is only ever one.
        """
...
def azure_client(env_path: str | Path = AZURE_ENV_PATH, *, max_tokens: int = 1200,
    ...
    before it emits anything, and `acr.llm` records a run where the thinking consumed the
