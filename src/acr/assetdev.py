...
from .spec import ProvenanceRecord, load_spec
...
ID_DOMAIN = b"acr.assetdev.patient_digest/1"
...
class Labelling:
    ...
    Redeclared here rather than imported from `acr.labelling` so the two modules are coupled by
...
class Split:
    ...
    def to_dict(self) -> dict:
        return {"kind": "acr.assetdev.split/1", "split_hash": self.split_hash, "seed": self.seed,
...
class Certification:
    ...
    def to_dict(self) -> dict:
        ...
        return {**d, "kind": "acr.assetdev.certification/1", "verdict": self.verdict,
...
def _verify(spec: Any, element: str, value: list[str]) -> None:
    ...
    record is about it."""
    from .spec import _content_hash
...
def _emit_proposal(cert: Certification, spec_path: str | Path, proposals_dir: str | Path | None,
    ...
    _write(path, yaml.safe_dump({
        "kind": "acr.assetdev.stratum_proposal/1",
