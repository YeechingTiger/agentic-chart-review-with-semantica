...
from .prose import (ICDO_NAMES, format_is_registry_notation, human_field, human_list, plain,
                    quoted, slug)
...
def _editorial(spec) -> tuple[dict, list]:
    """`acr.spec.editorial_records`, tolerant of a spec object that has no editorial channel.

    Imported inside the call rather than at module scope: `acr.spec`'s own docstrings point
...
def editorial_problems(spec) -> list:
    ...
    The renderer asks this module rather than `acr.spec` directly, for the same reason
