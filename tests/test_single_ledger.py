...
def test_flat_ledger_is_gone_not_merely_unused():
    assert not hasattr(acr.state, "CoverageLedger"), (
    ...
    assert not hasattr(acr.state, "check_proof_obligation"), (
        "the flat gate went with it; the stratified ledger is what the gate consumes now"
    )


def test_coverage_is_the_only_definition():
    assert hasattr(acr.coverage, "CoverageLedger")
    src = inspect.getsource(acr.state)
...
def test_the_ledger_reports_which_mode_it_is_in():
    ...
    bare = acr.coverage.CoverageLedger(docs, [])
    assert bare.to_dict()["mode"] == "unstratified"

    strata = acr.coverage.strata_from_spec(
        __import__("acr.spec", fromlist=["load_spec"]).load_spec(
