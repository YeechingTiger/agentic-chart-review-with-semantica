...
def test_the_seam_check_reports_a_deterministic_dimension_as_not_permitted():
    ...
    assert "acr.review.answer_gate.check_gate" in problems["task_completion"]
...
def test_every_deterministic_verifier_names_code_that_actually_exists(dim):
    ...
    Checked by AST rather than by import: resolving `acr.review.answer_gate.check_gate` through the
