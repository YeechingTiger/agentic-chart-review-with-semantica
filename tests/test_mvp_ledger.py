...
def _write_run(run_dir: Path) -> None:
    ...
    events = [
        ...
        {"seq": 2, "ts": "t", "kind": "tool_call", "tool": "note_decision",
         "args": {"facing": "no evidence gathered yet",
                  "decision": "submit from memory", "because": "overconfidence",
                  "used": ["note:Surgical-Pathology-Document_2023-04-12"]},
         "result": {"noted": True, "n_decisions": 1, "used": [{"ref": "note:Surgical-Pathology-Document_2023-04-12",
        ...
        {"seq": 4, "ts": "t", "kind": "tool_call", "tool": "note_decision",
         ...
         "result": {"noted": True, "n_decisions": 2, "used": [{"ref": "rule:evidence_rules", "kind": "rule",
...
def test_semantica_chain_walks_result_gate_submission(tmp_path: Path):
    ...
    steps = [row for row in chain if row["category"] == "step"]
    assert [s["outcome"] for s in steps] == [
        "record the pathology span, then resubmit", "submit from memory"]  # nearest first
    assert [s["category"] for s in steps] == ["step", "step"]   # unclassified by design
    ...
    rows = ledger.decisions(category_prefix="step")
    assert len(rows) == 2
    later = rows[-1]
    assert later["outcome"] == "record the pathology span, then resubmit"
    assert later["case_id"] == "SYN0001"
    assert later["context"] == {"n_searches": 0, "n_evidence": 0}
    assert later["used"] == ["rule:evidence_rules"]
    assert ledger.decisions(category_prefix="gate:nothing") == []
...
def test_live_recording_equals_replaying_the_trace(tmp_path: Path):
    ...
    server.call("note_decision", {"facing": "f",
