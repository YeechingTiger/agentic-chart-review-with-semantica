...
## 0 · 目的：trace 是探针，产物是**问题**

这套东西的用途，一句话：

> **看模型到底用了什么证据、考虑得全不全，然后反过来倒逼人类把没说清的地方说清楚。**

`THE_IDEAL_SYSTEM.md` 的生命周期主张说的是同一件事：**agent 是著合同期的探针。**

为什么探针有效——**人类审录员遇到含糊时会默默决定然后走开**，那个决定不留痕，所以
manual 永远不会补上它（决策沉淀设计 §1.3 把这叫"最要命的不一致"）。
**被强制叙述的 agent 做不到默默决定**：每一个这样的时刻都会变成一条
`grounding=own_knowledge` 的记录。

所以这套设计的产物**不是规则**——规则是人写的。产物是**一份人不得不回答的问题清单**。
...
四字段语义    ── 保证同类的实例真的能放一起比 ─┤
                                              ├─► 分歧可算
run 分解      ── 保证每一步都有决策点 ────────┤
依据出处      ── 凭的是我们给的还是它自己的 ───┴─► **一致但无依据**也抓得到
                                                  ⇓
                              §9 完备性阶梯（考虑得全不全）
                              §10 问题清单（倒逼的具体形式）
```

**依据出处是最要紧的一件**：一个判断若凭模型自带的知识，它就发生在合同没覆盖、没人批准
过的地方。文档 §4.2 把这叫"最危险的角落——规范不清晰 + 执行一致 = 隐性立法"，并说
**分歧检测器抓不到它**。记下依据出处，是唯一能抓到它的办法。
...
## 9 · "考虑得全不全"：一条服务端可算的完备性阶梯

"全面"不是感觉，是四级落差。**每一级都是服务端事实，不需要模型配合，也不需要金标准。**

```
浮现  N_surfaced   搜索命中 + 列表返回的 note_id 并集
  ↓  落差①：浮现了但没打开
读过  N_read       documents_read
  ↓  落差②：读了但没留下判断
判过  N_judged     record_finding 覆盖的 note_id
  ↓  落差③：判了但没进最终答案
引用  N_cited      终局 used 里的 note_id
```

| 落差 | 意味着什么 | 是不是问题 |
|---|---|---|
| ① 浮现未读 | 搜到了，没打开 | **常见的漏证据形态**。要问："这些你为什么没打开？" |
| ② 读了未判 | 判断蒸发了 | **总是问题**。coverage 的分母缺了这块 |
| ③ 判了未引用 | 读了、判为无关 | **健康的**——这一级落差大恰恰说明认真看了 |

外加一条正交的，来自 CONTEXT.md 的 **Complete Answer** 定义：

```
合同声明的 discriminating facts   D_declared
被查证过的                        D_checked        ← used 里出现过对应 fact 的证据
```

> **一个答案可以是对的但不完备**（走了个在这张 chart 上恰好管用的捷径）——
> 而同一个捷径在镜像 chart 上是错的。`D_checked < D_declared` 就是这种情况的签名。

四级落差 + 一条完备性比，就是"考虑得全不全"的全部可操作形式。

---

## 10 · 倒逼的具体形式：一份问题清单

`precipitate` 现在输出的是**分歧报告**，那还差一步——报告要人去解读，而**问题**是人可以直接
回答的。这一步的转换是整套设计的出口。

### 10.1 差别

```
分歧报告   "coverage 这一类有 3 次决策，2 次这样 1 次那样，输入重叠 1.0，判断分歧"
问题       "影像的 'suspicious for' 措辞，算不算能确立肺癌诊断？
            9 个病人里出现，agent 7 次判不算、2 次判算，全部凭自己的临床知识——
            合同没写这条。你答了之后，这 9 个病人需要重跑。"
```

第二种，临床专家三十秒能答。第一种，他要先学会读这张表。

### 10.2 一个问题条目的字段

| 字段 | 内容 | 从哪来 |
|---|---|---|
| **问题** | 一句可直接回答的话 | 由 `(type, slug)` 模板化生成 |
| **情形** | 抽象的局面 | `scenario`（铁律已保证它是抽象的） |
| **出现** | 几个病人、几次运行 | 账本计数 |
| **agent 怎么判的** | outcome 分布 + 各自的 reasoning | 账本 |
| **凭什么判的** | grounding 分布 | 账本 |
| **回答后影响谁** | 命中这个 point 的历史病例 | `get_affected_decisions` |
| **优先级** | `diversity × (0.5+impact) × frequency × safety` | 计算 |

同一个 `(type, slug)` 的实例**打包成一个裁决包**——一次会议消化一个点，不是一个实例
（决策沉淀设计 §5.5：专家时间是最稀缺资源）。

### 10.3 问题的三个来源，第二个最值钱

| 来源 | 触发条件 | 问句形态 |
|---|---|---|
| **分歧** | 同 `(type, slug)`，outcome 不一致 | "这两种判法哪个对？" |
| **一致但无依据** ★ | 同 `(type, slug)`，outcome **一致**，但 grounding 集中在 `own_knowledge` | **"你们默认了什么？请写下来。"** |
| **不完备** | §9 的落差①、②，或 `D_checked < D_declared` | "这些是该看的吗？" |

★ **第二个来源是这套设计相对于所有分歧检测方法的真正增量。**

一个模型每次都用同一条自创规则、结果高度一致——分歧是零，熵是零，任何基于分歧的方法都
认为这里很健康。而这恰恰是文档 §4.2 的"最危险角落"：**规范不清晰 + 执行一致 = 隐性立法**。
只有 grounding 抓得到它，而且抓到的形态正好就是一句最该问的话：
**"这条规则是模型自己立的，你们同意吗？"**

### 10.4 一条纪律

问题清单**只提问，不拟答**。生成一个"建议的规则措辞"看起来贴心，实际是把模型的习惯
洗成政策——`CHART_REVIEW_KNOWLEDGE_AND_SEARCH_LAYERS.md` 的原话：
**"Experience never becomes guideline merely because it repeats."**

出口是问题；答案必须来自规范性来源或签字的人类语义决定。
